...
class DccTicketingAllowListRepositoryTest : BaseTest() {
    ...
    @Test
    fun `returns empty container if storage is empty`() = runTest2(ignoreActive = true) {
    ...
    @Test
    fun `clear() clears internal data and storage`() = runTest2(ignoreActive = true) {
        with(createInstance(scope = this)) {
            refresh()
    ...
    @Test
    fun `fallbacks to empty container if initial data loading fails`() = runTest2(ignoreActive = true) {
    ...
    @Test
    fun `fallbacks to last container if anything fails`() = runTest2(ignoreActive = true) {
        coEvery { dccTicketingAllowListStorage.load() } returns byteArrayOf()
    ...
    @Test
    fun `updates internal data and storage with new data`() = runTest2(ignoreActive = true) {
