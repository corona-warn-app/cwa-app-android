...
class QrCodeCameraImageParserTest : BaseTest() {
    ...
    @Test
    fun `finds certificate`() {
        val path = File("./src/test/dcc-test-data")
        if (path.exists()) lookForImagesAndDecode(path)
        else throw InvalidPathException("./src/test/dcc-test-data", "Directory with test data is missing")
    }
    ...
    internal fun Image.toGrayU8(): GrayU8 {
