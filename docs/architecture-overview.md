...
This document outlines the architecture of the CWA mobile client. It does not necessarily reflect the current implementation status of this repository and will be enriched in the future as it is still in development.
...
The general transaction logic is implemented in the abstract `Transaction` class. The subclasses implement the logic containing the states of the transaction. For details regarding states, please refer to the documentations.
